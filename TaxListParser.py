...
#
# ---------------------------------------------------------------------------
# 2023-10-20 revisions:
# - Python 3, via 2to3; formatting; minor error handling
...
class TaxListParser:
        ...
        try:
            if field in self.tags:
                ...
                if field == "block" or field == "lot":
                    zeroStrip = re.compile(
                        r"^[0]*(\w+$)"
                    )  # this is a kludge and should probably be rewritten to match two groups of out the "00000SS00" format for decimalized blocks and lots
                    if value.isalpha():
                        return value
                    if len(value) == 5:
                    ...
                    #    return str(int(value)) # drop padded zeroes
                        ...
                        zeroStrip = re.compile(
                            r"^[\s]*(\w+$)"
                        )  # change the regex for the lot to only look for spaces.
                    ...
                    if len(value) == 6:
                        if value == "000000" or value == "      ":
                            ...
                            if int(value[-2:]) > 25:  # last two digits
            ...
            sys.stderr.write(f"{stri}/{endi}: {value}\n")
            return value
    ...
    def genCreateTableMySQL(self, tablename):
        text = (
            "CREATE TABLE `"
            + tablename
            + "` ( `recordid` INT NOT NULL UNSIGNED AUTO_INCREMENT, \n"
            + "`pams_pin` VARCHAR(30),\n"
            + "`source` VARCHAR(20),\n"
    ...
    def genInsertMySQL(self, table, source):
        ...
        text = "INSERT INTO %s SET `pams_pin` = '%s', `source` = '%s', " % (
            table,
            pams,
            source,
        )
    ...
    def genExecuteManyInsert(self, tablename):
        ...
        sql += ", ".join(
            ["`%s`" % x for x in self.fields]
        )  # map/lambda to wrap keys in MySQL field delimiters
    ...
    def dateTest(self, datestr):
        ...
        try:
            ...
            dateOk = datestr == result.strftime(
                format
